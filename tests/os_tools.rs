...
fn test_database_initialization() {
    ...
    println!("Supported OS: {:?}", db.os_specific.keys().collect::<Vec<_>>());
}
...
fn test_tool_categories() {
    ...
    assert!(text_tools.iter().any(|t| t.name == "grep" || t.name == "findstr"));
...
fn test_admin_requirements() {
    ...
    let non_admin_count = db.tools.values()
        .filter(|tool| !tool.requires_admin)
        .count();
    
    let admin_count = db.tools.values()
        .filter(|tool| tool.requires_admin)
        .count();
    
    println!("Non-admin tools: {}, Admin tools: {}", non_admin_count, admin_count);
...
fn test_tool_recommendations() {
    ...
    assert!(file_recs.iter().any(|t| t.category == ToolCategory::FileSystem));
    
    // Text processing
    let text_recs = db.get_recommended_tools("search for text in files");
    assert!(!text_recs.is_empty());
    assert!(text_recs.iter().any(|t| t.category == ToolCategory::TextProcessing));
    
    // Network operations
    let net_recs = db.get_recommended_tools("test network connection");
    assert!(!net_recs.is_empty());
    assert!(net_recs.iter().any(|t| t.category == ToolCategory::NetworkTools));
}
...
fn test_tool_examples() {
    ...
    for (name, tool) in &db.tools {
        ...
        for example in &tool.examples {
            assert!(!example.description.is_empty(), "Tool {} has empty example description", name);
            assert!(!example.command.is_empty(), "Tool {} has empty example command", name);
        }
    }
}
...
fn test_comprehensive_coverage() {
    ...
    for category in expected_categories {
        ...
        assert!(!tools.is_empty(), "No tools found for category {:?}", category);
    }
