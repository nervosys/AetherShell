...
fn test_simon_self_describing() {
    ...
    match result {
        Ok(name) => {
            ...
            assert!(cap_names.iter().any(|n| n.contains("system") || n.contains("cpu") || n.contains("gpu")),
    ...
    if let Ok(name) = registry.register_self_describing(simon_path, Some("simon")) {
        ...
        match result {
            Ok(res) => {
                ...
                assert!(res.success || res.error.is_some(), "Should get result or error");
            }
            Err(e) => {
