...
fn the_uncoded_failure_count_does_not_grow() {
    ...
    // A ratchet over the whole catalogue. It exists because ~750 error-handling
    // fixes brought this from 157 to ZERO: every builtin in the catalogue,
    // handed an argument none of them accept, now answers with a code an
    // agent can branch on. Nothing was stopping that coming back.
    //
    ...
    // reports the split.
    // Zero within this scope, and zero catalogue-wide, tracked by
