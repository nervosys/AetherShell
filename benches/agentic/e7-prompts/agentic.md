Write the command in **AetherShell's agentic syntax**, run as `ae -a -c '<command>'`.
Output only the command itself (not the `ae -a -c` wrapper).
...
And this is the agentic syntax reference:
