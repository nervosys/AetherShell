...
## The NERVOSYS stack: IronGate, IronWorks, IronVault

AetherShell is a frontend. It does not decide which model serves a request, it
does not run inference, and it does not own where weights live — three other
projects do:
...
Point `AETHER_AI` at the gateway and every AI call routes through it:

```powershell
$env:AETHER_AI = "irongate"     # or leave unset — a running gateway is found
```

`irongate:` names a **virtual model**, not a vendor model. Ask for `auto` and
the gateway decides: routine edits to a free local model on IronWorks, planning
work to a frontier one, with an escalation path when the cheap answer comes back
short.

```ae
ai("Explain this stack trace")            # routed by difficulty
"irongate:fast"                           # or name the virtual model yourself
"irongate:deep"
```

Because routing is a decision someone has to be able to audit, the shell shows
you what the gateway did:

```ae
ai_gateway()
# {url: "http://localhost:7700/v1", reachable: true, providers: 1,
#  models: ["auto", "fast"], targets: [...], default_model: "auto"}

ai_gateway().targets | first
# {target: "local-llama:qwen2.5-coder-7b", circuit: "closed",
#  in_flight: 0, avg_latency_ms: 344}
```

**Local inference means IronWorks behind the gateway.** The default build
compiles no in-process backend at all, so `ai_local_generate` routes through
IronGate and reports which engine actually answered:

```ae
ai_local_generate("auto", "Say OK")
# {text: "OK", backend: "local-llama:qwen2.5-coder-7b",
#  prompt_tokens: 7, completion_tokens: 1, generation_ms: 658.6, ...}
```

(`--features candle` still gives you in-process GGUF inference; the gateway is
the fallback, not a replacement.)

**Models and conversion live in IronVault**, reached through its `iv` CLI:

```ae
vault_models()                            # what the vault is storing
vault_conversions()                       # conversions it really supports
vault_convert({name: "my-llm", to_format: "gguf", quantization: "q4_k_m"})
```
...
| --- | --- | --- |
| `AETHER_AI` | *(unset)* | `irongate` to route through the gateway |
| `IRONGATE_URL` | `http://localhost:7700/v1` | Gateway endpoint |
| `IRONGATE_MODEL` | `auto` | Virtual model when none is named |
| `IRONGATE_API_KEY` | *(unset)* | Only when the gateway runs `require_auth = true` |
| `IRONVAULT_BIN` | `iv` | Path to the vault CLI |

Nothing here is required: with no gateway running, every existing provider
(`openai:`, `anthropic:`, `ollama:`, …) works exactly as before.
