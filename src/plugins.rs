...
impl PluginRegistry {
    ...
    fn add_default_load_paths(&mut self) {
        ...
        if let Some(home) = dirs::home_dir() {
            self.load_paths.push(home.join(".aethershell").join("plugins"));
        }
        
        // XDG data directory
        if let Some(data_dir) = dirs::data_dir() {
            self.load_paths.push(data_dir.join("aethershell").join("plugins"));
        }
        
        // System plugin directory (Unix-like)
        #[cfg(unix)]
        self.load_paths.push(PathBuf::from("/usr/share/aethershell/plugins"));
        
        // System plugin directory (Windows)
        #[cfg(windows)]
        if let Some(program_data) = std::env::var_os("ProgramData") {
            self.load_paths.push(PathBuf::from(program_data).join("AetherShell").join("plugins"));
        }
    }
    ...
    pub fn register_ai_backend(&mut self, plugin: Arc<dyn AIBackendPlugin>) -> Result<()> {
        ...
        // Register main entry
        self.plugins.insert(plugin_id.clone(), PluginEntry {
    ...
    pub fn register_builtin(&mut self, plugin: Arc<dyn BuiltinPlugin>) -> Result<()> {
        ...
        for name in plugin.builtin_names() {
            self.builtins.insert(name, (plugin_id.clone(), plugin.clone()));
        }
    ...
    pub fn register_file_handler(&mut self, plugin: Arc<dyn FileHandlerPlugin>) -> Result<()> {
        ...
        // Register main entry
        self.plugins.insert(plugin_id, PluginEntry {
            metadata: metadata.clone(),
            enabled: true,
            load_time: std::time::Instant::now(),
        });
        ...
        for ext in plugin.supported_extensions() {
            self.file_handlers.insert(ext.to_lowercase(), plugin.clone());
        }
    ...
    pub fn is_enabled(&self, plugin_id: &str) -> bool {
        self.plugins.get(plugin_id).map(|e| e.enabled).unwrap_or(false)
    }
...
fn json_to_value(json: serde_json::Value) -> Value {
    match json {
        ...
        serde_json::Value::Object(obj) => {
            let map = obj.into_iter().map(|(k, v)| (k, json_to_value(v))).collect();
            Value::Record(map)
        }
    }
}
...
fn value_to_toml(value: &Value) -> toml::Value {
    match value {
        ...
        Value::Table(t) => {
            ...
            let tables: Vec<toml::Value> = t.rows.iter().map(|row| {
...
pub fn bi_plugin_info(plugin_id: &str) -> Value {
    ...
    match registry.get_metadata(plugin_id) {
        Some(meta) => {
            ...
            rec.insert("author".to_string(), Value::Str(meta.author));
            rec.insert("description".to_string(), Value::Str(meta.description));
            rec.insert("min_aether_version".to_string(), Value::Str(meta.min_aether_version));
            rec.insert(
                "categories".to_string(),
                Value::Array(
                    meta.categories
