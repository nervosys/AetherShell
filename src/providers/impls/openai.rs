    ...
    fn build_messages(&self, messages: &[Message]) -> Vec<JsonValue> {
        messages.iter().map(|msg| {
            ...
            if let Some(ref tool_calls) = msg.tool_calls {
                msg_obj["tool_calls"] = json!(tool_calls.iter().map(|tc| json!({
                    "id": tc.id,
                    "type": "function",
                    "function": {
                        "name": tc.name,
...
impl LLMProvider for OpenAIProvider {
    ...
    fn parse_response(&self, response: &JsonValue) -> Result<ChatResponse, ProviderError> {
        let choice = response["choices"].get(0)
            .ok_or_else(|| ProviderError::Unknown { message: "No choices in response".to_string() })?;
