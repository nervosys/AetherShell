    ...
    match expr {
        ...
        Expr::Array(items) => {
            let vals: Result<Vec<Value>, String> =
                items.iter().map(|e| eval_expr_wasm(e, env)).collect();
            Ok(Value::Array(vals?))
        }
        ...
        Expr::UnaryNot(inner) => match eval_expr_wasm(inner, env)? {
            Value::Bool(b) => Ok(Value::Bool(!b)),
            _ => Err("! requires boolean".to_string()),
        },

        Expr::UnaryMinus(inner) => match eval_expr_wasm(inner, env)? {
...
fn eval_call_wasm(
    ...
    let mut evaled: Vec<Value> = args
        .iter()
        .map(|a| eval_expr_wasm(a, env))
        .collect::<Result<_, _>>()?;
    ...
    match func {
        ...
        "len" => match evaled.first().ok_or("len requires 1 arg")? {
        ...
        "map" => {
            let (arr, func) = (
                evaled.first().ok_or("map requires array")?,
                evaled.get(1).ok_or("map requires function")?,
            );
            match (arr, func) {
                (Value::Array(items), Value::Lambda(lam)) => {
                    let results: Result<Vec<Value>, String> = items
                        .iter()
                        .map(|item| {
                            let mut inner = env.clone();
                            if let Some(p) = lam.params.first() {
                                inner.set(p.clone(), item.clone());
                            }
        ...
        "where" | "filter" => {
            let (arr, func) = (
                evaled.first().ok_or("where requires array")?,
                evaled.get(1).ok_or("where requires function")?,
            );
            match (arr, func) {
                (Value::Array(items), Value::Lambda(lam)) => {
                    let results: Result<Vec<Value>, String> = items
        ...
        "reduce" => {
            let (arr, func, init) = (
                evaled.first().ok_or("reduce requires array")?,
                evaled.get(1).ok_or("reduce requires function")?,
                evaled.get(2).ok_or("reduce requires initial")?,
            );
            match (arr, func) {
                (Value::Array(items), Value::Lambda(lam)) => {
        ...
        "take" => match (evaled.first(), evaled.get(1)) {
            (Some(Value::Array(items)), Some(Value::Int(n))) => Ok(Value::Array(
        ...
        "any" => {
            let (arr, func) = (
                evaled.first().ok_or("any requires array")?,
                evaled.get(1).ok_or("any requires function")?,
            );
            match (arr, func) {
                (Value::Array(items), Value::Lambda(lam)) => {
                    for item in items {
                        ...
                        if let Value::Bool(true) = eval_expr_wasm(&lam.body, &mut inner)? {
        ...
        "all" => {
            let (arr, func) = (
                evaled.first().ok_or("all requires array")?,
                evaled.get(1).ok_or("all requires function")?,
            );
            match (arr, func) {
                (Value::Array(items), Value::Lambda(lam)) => {
                    for item in items {
                        ...
                        if let Value::Bool(false) = eval_expr_wasm(&lam.body, &mut inner)? {
        ...
        "split" => match (evaled.first(), evaled.get(1)) {
            (Some(Value::Str(s)), Some(Value::Str(d))) => Ok(Value::Array(
                s.split(d.as_str())
        ...
        "replace" => match (evaled.first(), evaled.get(1), evaled.get(2)) {
            (Some(Value::Str(s)), Some(Value::Str(f)), Some(Value::Str(t))) => {
        ...
        "starts_with" => match (evaled.first(), evaled.get(1)) {
            (Some(Value::Str(s)), Some(Value::Str(p))) => {
        ...
        "ends_with" => match (evaled.first(), evaled.get(1)) {
            (Some(Value::Str(s)), Some(Value::Str(su))) => {
                Ok(Value::Bool(s.ends_with(su.as_str())))
            }
        ...
        "slice" => match (evaled.first(), evaled.get(1), evaled.get(2)) {
            (Some(Value::Array(items)), Some(Value::Int(s)), Some(Value::Int(e))) => Ok(
                Value::Array(items[*s as usize..(*e as usize).min(items.len())].to_vec()),
            ),
            (Some(Value::Str(str)), Some(Value::Int(s)), Some(Value::Int(e))) => Ok(Value::Str(
                str[*s as usize..(*e as usize).min(str.len())].to_string(),
            )),
        ...
        "unique" => match evaled.first() {
            Some(Value::Array(items)) => {
                ...
                for item in items {
                    if !seen.iter().any(|x| values_equal(x, item)) {
        ...
        "to_string" => Ok(Value::Str(format_value(
            evaled.first().ok_or("to_string requires value")?,
        ))),
...
impl AetherWasm {
    ...
    pub fn parse(&self, _code: &str) -> Result<String, String> {
