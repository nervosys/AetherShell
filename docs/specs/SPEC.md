...
```ae
files = ls "."           // files: Table (type inferred)
count = 42               // count: Int
items = [1, 2, 3]        // items: Array<Int>
```
...
* Alternative syntax with explicit `let`:

```ae
let files = ls "."       // Using let keyword
let mut total: Int = 0   // Mutable with type annotation
```
